 ...
 * @since 04-06-2020
 */
public class PostProcessorImpl implements CommandProcessor {
	private static final Logger LOG = LoggerFactory.getLogger(PostProcessorImpl.class);

	private List<LinuxCommand> commands;

	public PostProcessorImpl(List<LinuxCommand> commands){
	...
	public void execute(){
		for (LinuxCommand command: commands){
			try {
				...
				LOG.info("****Post-processing command completed with exit code: " + exitCode);
